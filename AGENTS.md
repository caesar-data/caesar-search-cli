...
- `read <url>` may render the page with your own local browser and return it directly, marked by a `local_render` warning. A local render is not a server capture, so it has **no `doc_id`** — re-read by URL, or use `--no-local-render` for a server read with a `doc_id`. It is a reduced envelope: it honors `--include` for `metadata`/`content`, but `--include passages`/`capture_history` (like `--query`, `--start-char`, and a `doc_id` argument) always go to the server, and `request_id`/`session_id`/`access` are absent because no server request was made.
