...
- A `--start-char` continuation of a URL read stays local: it re-renders the live page and slices the new extraction, so offsets are consistent with the read that reported `truncated: true` unless the page itself changed between reads. Offsets cross extractions when the two reads took different paths — the continuation falling back (`local_render_fallback`) or the FIRST read having been served by the server; in either case re-read from `--start-char 0` rather than stitching.
- A page behind a bot/CAPTCHA wall falls back to the server automatically (`local_render_fallback` — the server's crawler may hold a good capture). A `bot_wall_skipped` warning means the server could not produce it either: `read` exits `0` with **empty content**. Treat it as a skip, not a successful read — branch on the warning `code`, not the exit code alone, and try a different source.
